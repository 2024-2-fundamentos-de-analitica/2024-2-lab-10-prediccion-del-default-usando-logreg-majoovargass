...
def preprocess_data(dfp):
    dfp = dfp.rename(columns={'default payment next month': 'default'})
    dfp.drop(columns=['ID'], inplace=True, errors='ignore')
    dfp.dropna(inplace=True)
    dfp = dfp[(dfp["EDUCATION"] != 0) & (dfp["MARRIAGE"] != 0)]
    dfp["EDUCATION"] = dfp["EDUCATION"].apply(lambda x: 4 if x > 4 else x)
    return dfp
...
def tune_hyperparameters(pipeline, x_train, y_train):
    ...
    return GridSearchCV(
        pipeline, param_grid=pm, cv=10, scoring='balanced_accuracy', n_jobs=-1, refit=True
    )
...
def save_trained_model(pfile, estimator):
...
def main():
    ...
    ft = x_train.select_dtypes(include=['object', 'category']).columns.tolist()
    
    pipeline = create_pipeline(ft)
